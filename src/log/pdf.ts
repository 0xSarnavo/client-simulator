...
<p>Generated by leakdown${model ? ` (${esc(model)})` : ""}. Findings are risk signals from simulated visitors, not measured traffic.</p>
${body}
</body></html>`;
}
...
export async function htmlToPdf(html: string, outPath: string): Promise<string> {
  ...
  try {
    ...
    await page.pdf({
      ...
      footerTemplate:
        '<div style="font-size:8px;color:#888;width:100%;text-align:center;">leakdown · simulated prospects: risk signals, not measured traffic · <span class="pageNumber"></span>/<span class="totalPages"></span></div>',
