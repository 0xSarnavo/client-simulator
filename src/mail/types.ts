...
export function extractCodes(...parts: string[]): string[] {
  ...
  // Not every one-time code is digits — site-g's login mail sends a 32-char
  ...
  for (const m of haystack.matchAll(ALNUM_CODE_RE)) {
    const token = m[1];
    // A cue word alone is not enough: "…or the code. site-g inc." would
    // offer "site-g". Real codes carry a digit, mixed case, or all-caps —
